			...
			<Card.Root class="gap-0 overflow-hidden p-0">
				...
				{#if followUps.length}
					{#each followUps as deal (deal.path)}
						...
						<div class="border-border flex items-center border-b last:border-0">
							<Clickable class="hover:bg-accent flex flex-1 cursor-pointer items-center gap-4 px-5 py-3.5" onclick={() => go({ name: 'deal', path: deal.path })}>
			...
			<Card.Root class="gap-0 overflow-hidden p-0">
				...
				{#if recent.length}
					{#each recent as it (it.name)}
						...
						<Clickable
							class="border-border hover:bg-accent flex cursor-pointer items-start gap-3 border-b px-5 py-4 last:border-0"
						...
						>
							...
							<div class="flex min-w-0 flex-1 flex-col gap-1">
								...
								{#if it.summary}<p class="text-muted-foreground text-[13px] leading-snug">{it.summary}</p>{/if}
							</div>
						</Clickable>
					{/each}
				{:else}
